...
2. Adjust the parameters. Every control is live. The chunking and wavelet sliders re-run the pipeline when you release them, and the occlusion volume controls rebuild the volume while you drag.
   ...
   - **Gaussian Splat Mode**, shown when the source is a 3D Gaussian Splatting scene (a `.ply` with scales, rotations and opacities, or a `.splat`), bakes every point as the full Gaussian instead of an oriented disc: its three scales and rotation, its opacity, an 8-bit colour and its spherical harmonics. The package is larger (20 bytes per point plus the harmonics, against 8 for a surfel) and the renderer draws it the way a reference splat viewer does. On by default for such sources. **SH Quality** picks how much view-dependent colour to keep: Low keeps the first band (9 bytes per point), High keeps all three (45 bytes).
