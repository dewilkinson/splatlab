...
2. Adjust the parameters. Every control is live. The chunking and wavelet sliders re-run the pipeline when you release them, and the occlusion volume controls rebuild the volume while you drag.
   ...
   - **Gaussian Splat Mode**, shown when the source is a 3D Gaussian Splatting scene (a `.ply` with scales, rotations and opacities, or a `.splat`), bakes every point as the full Gaussian instead of an oriented disc: its three scales and rotation, its opacity, an 8-bit colour and its spherical harmonics. The package is larger (20 bytes per point plus the harmonics, against 8 for a surfel) and the renderer draws it the way a reference splat viewer does. For now the renderer draws every level of such a package, level 0 included, as soft camera-facing discs sized from each level's point spacing, with the usual dithered level transitions; the Gaussian records still carry the colours. On by default for such sources. **SH Quality** picks how much view-dependent colour to keep: Low keeps the first band (9 bytes per point), High keeps all three (45 bytes).
...
- **Refinement Visualizer**, on by default, tints every chunk as it arrives, also in Gaussian splat mode. The newest chunks glow, settle to the regular tint and fade out. Sliders set the fade duration, glow intensity and hue. Press **Evict** on the residency panel and watch the model grow back.
- **GPU Silhouette Edge Refinement** keeps outlines crisp while the interior of the model is still coarse.
- **Splat Radius Scale**. Larger splats fill gaps in sparse data but look blobby up close. 1.0x is the place to start. **Auto Splat Size**, on by default, sizes discs from each level's measured point spacing. Neither applies in Gaussian splat mode, where every Gaussian carries its own size.
- **Splat Blending**, shown for Gaussian splat packages. **Display space** blends the stored colours as they are, which is what the training rasterizer and SuperSplat do, so the picture matches those viewers. **Linear** decodes the colours first and blends in linear light, which is physically tidier and slightly different. In this mode the splats are sorted back to front every frame on the GPU, the occlusion volume is not used, and Detach Camera drops what the frozen camera culls instead of painting it red.
- **Match Level 0 Softness**, shown greyed out for Gaussian splat packages while the Gaussian level 0 is switched off (its saved value still sizes the discs); on by default. The coarser levels of a splat package are drawn as camera-facing discs, and this sizes those discs from the level 0 Gaussians themselves: the read-out beside the checkbox shows how wide the typical level 0 Gaussian is relative to the level 0 point spacing (the softness ratio) and the disc radius, as a multiple of each level's own spacing, that gives the discs the same softness. Discs of level 1 and up also take an opacity floor so a faint record does not read as a hole. The effect is a streamed model that reads as one surface while it refills, with a less visible hand-over from discs to Gaussians. Off, the discs are 1.5 times the level spacing with each record's opacity as is. Remembered across sessions.
- **Detach Camera** (Ctrl+D) freezes the culling camera where the view is and steps the view aside so you can inspect the culling from any angle. The model is exactly what the frozen camera would render, its level of detail included: surfels it would draw keep their colour, mid grey where you see their backs, and everything it would have culled, including the far half of the model beyond its centre, stays on screen as a translucent red volume, depth-tested against your view so it never builds up. Streaming keeps running from the frozen camera's point of view, so Evict refills the model as that camera would see it. The occlusion volume shows only the faces turned toward the frozen camera, and its frustum is drawn.
...
**Refinement pacing.** New cross-fades start at a rate that keeps the number of chunks mid-fade under a soft cap, so a fast refill dissolves chunk by chunk instead of snapping; a refinement that has to wait keeps showing its parent and starts a frame or two later, while the data keeps arriving at full speed. The tab reports the active transitions and the starts deferred this frame.
